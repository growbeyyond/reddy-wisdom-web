...
-- 2. Create user_roles table with proper security (if not exists)
...
-- 3. Migrate existing roles from profiles to user_roles (if role column exists)
DO $$
BEGIN
  IF EXISTS (
...
DROP POLICY IF EXISTS "Admins and staff can view newsletter subscriptions" ON public.newsletter_subscriptions;
DROP POLICY IF EXISTS "Admins and staff can update newsletter subscriptions" ON public.newsletter_subscriptions;
...
DROP POLICY IF EXISTS "Admins and staff can update contact submissions" ON public.contact_submissions;
...
DROP POLICY IF EXISTS "Admins and staff can view all patient documents" ON public.patient_documents;
DROP POLICY IF EXISTS "Admins and staff can update patient documents" ON public.patient_documents;
DROP POLICY IF EXISTS "Admins can delete patient documents" ON public.patient_documents;
